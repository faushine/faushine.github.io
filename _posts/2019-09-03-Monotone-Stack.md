...
(2) `i = 1`，data[1] = 2，栈顶元素0对应data元素7大于2，此时`0出栈`，计算结果：`data[0]*(prefix[1]-prefix[-1+1]) = 49`，再将`1`压栈。
...
(5) `i = 4`，data[4] = 5，栈顶元素3对应data元素6大于5，此时`3出栈`，计算结果：`data[3]*(prefix[4]-prefix[2+1]) = 6*(19-13)=36`。因为现在栈顶元素2对应的data元素4小于5，所以停止计算，将4压栈。
